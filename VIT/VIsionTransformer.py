...
test_data = cifar10_test[:][0]
for idx in range (min(len(test_data),10)) :
  
  im = test_data[idx]

  img = im[0]
  im = 255 - im[0] # image 값 반전

  # encoder 에서 나온 attention 을 head 에 대해 평균.
  _, attentions = model(im.unsqueeze(0).to(device, torch.float))
  #_, attentions = model(im.unsqueeze(0).to(device, torch.float))
  x = (_.data).cpu()
  ...
  mask = cv2.resize(mask / mask.max(), (im.shape[2], im.shape[1]))[..., np.newaxis]
#  result = (mask.squeeze() * im.numpy()).astype("uint8")
  mask = rearrange(mask,'a b c -> c a b')
